 ...
 */
 router.post('/', function(req, res){
    ...
    switch(req.body.action) {
      ...
      break
      case 'tick_status':
      updateTickStatus(req.body.from, req.body.to, res)
      break

      case 'justOnline':
      updateSingleTickStatus(userId, res)
      break
 ...
 res.status(200).json( common.formatResponse({
   ...
   data: {
    ...
    function updateSingleTickStatus(from, res) {
  ...
  if(result1){
    ...
    res.status(200).json( common.formatResponse({
        type: 'success',
        code: 'CHAT_TICK_STATUS_SUCCESS',
        data: 'Chat Tick status updated'
      }))     
       ...
       function updateTickStatus(from, to, res) {
...
var from = new mongodb.ObjectId(from) 
   var to = new mongodb.ObjectId(to) 
        ...
        db.get().collection('chat_history').find({"to": from,"tick_status":"twotick"
 }).toArray(function(err, result1){
  ...
  if(result1){
    ...
    res.status(200).json( common.formatResponse({
        type: 'success',
        code: 'CHAT_SEEN_BLUETICK_SUCCESS',
   ...
   doc.chat_friends.map(function(n) {
    //console.log("nnnnnnnnnn",n)
     ...
     if(uId === iId) { 
       ...
       if(n.is_blocked == false){
        console.log("n.is_blocked == false")
        if(payload.msg != ""){
...
function accept(userId, inviteUserId, res) {
 ...
 checkAlreadyFriends(inviteUserId, userId, function(result) {
  console.log('----------result----------inviteUserId')
  console.log(result)
  if (!result) {
...
function addToFreindsList(userId, inviteUserId, callback) {
 ...
 db.get().collection('users').update({
  ...
  if(err){
   console.log("addToFreindsList",err)
 }else{            
   callback(true)
 }
})
